...
if ($_REQUEST["page"]=="")
	...
	############# Active #############
				if($_GET["act"]=="btnActive") {
					$strSqlActive = "select * from ".TBL_MAIN_CATEGORY." where intCat_id=".$_GET["id"];
					if($cnf->records_fetch($strSqlActive)==true) {
						...
						$strSqlUpdate = "update ".TBL_MAIN_CATEGORY." set `cStatus`='".$strActive."' where intCat_id=".$_GET["id"];
						$cnf->query_execute($strSqlUpdate);						
						
						header("Location:main_category.php?act=".$actmsg.$url_fields);
						} else {
						header("Location:main_category.php?act=Invalid".$url_fields);
					}
				}		
	...
	if($_REQUEST['act']== 'delete'){
		...
		$sql = "SELECT logo_image FROM `".TBL_MAIN_CATEGORY."` WHERE intCat_id=".$_GET["id"];
		$res = mysqli_query($con, $sql);
		$row = mysqli_fetch_object($res);
		@unlink("uploads/logo/".$row->logo_image);

		$strSql = "DELETE FROM ".TBL_MAIN_CATEGORY." where intCat_id=".$_GET["id"];

		$cnf->query_execute($strSql);

		header("Location:main_category.php?act=deleted");
...
if($_POST["btnSubmit"]!="") {		
				...
				$selOptions = $_POST["selOptions"];					
					...
					for($intI=0;$intI<=$IntCount;$intI++) {
						...
						$strSqlDelete = "delete from ".TBL_MAIN_CATEGORY." where intSubCatId !=0 AND intCat_id=".$_POST["chk"][$intI];
						
						$cnf->query_execute($strSqlDelete);						
						
						} else {
						$strSqlUpdateOptions = "update ".TBL_MAIN_CATEGORY." set ".$strString." where intCat_id=".$_POST["chk"][$intI]; 
					...
					header("Location:main_category.php?act=".$strString."&rec=mul");
				}				
							...
							<div class="col-lg-12">
								...
								<section class="panel">
                                    ...
                                    <form action="" method="POST" class="form-horizontal form-bordered" name="frm" onSubmit="return frmAction(frm);">
										...
										$strQry = "SELECT * FROM `".TBL_MAIN_CATEGORY."` WHERE intSubCatId = 0 ORDER BY `intCat_id` DESC";
                                            ...
                                            <table class="table table-bordered table-striped mb-none" id="datatable-default">
												...
												?>
                                                    ...
                                                    <tr class="gradeX">
                                                        ...
                                                        <td><?php echo $rowQry->vCatName; ?></td>

                                                        <td class="center"><?php echo $cnf->convetdate($rowQry->dtCreated); ?></td>

                                                        <td class="center"><?php echo $cnf->getActiveImageHide($rowQry->cStatus,'main_category.php?act=btnActive&id='.$rowQry->intCat_id)?></td>

                                                        <td class="center">

                                                        	
                                                            <a href="edit_main_category.php?id=<?php echo $rowQry->intCat_id.$url_fields ?>" onClick="return confirmMsg(1);" class="text-info" title="Edit"><i class="fa fa-pencil" ></i></a>&nbsp;&nbsp;
                                                            
                                                            <?php
                                                         if($rowQry->intSubCatId != 0){
                                                            ?>
                                                            <a href="main_category.php?act=delete&id=<?php echo $rowQry->intCat_id.$url_fields ?>" onClick="return confirmMsg(2);" class="text-danger" title="Delete"><i class="fa fa-trash-o"></i></a>
															<?php
														 }
															?>
                                                        </td>
                                            ...
                                            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="rgtlistbdr">
                                                ...
                                                <tr>
                                                    ...
                                                    <td width="200" align="left" valign="middle">
                                                        ...
                                                        <a href="main_category.php?act=<?php echo $strCheck?>" class="text_btm_link"><?php echo $strCheckString?></a>
