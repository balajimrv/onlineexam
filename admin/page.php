...
if ($_REQUEST["page"]=="")
	...
	############# Active #############
				if($_GET["act"]=="btnActive") {
					$strSqlActive = "select * from ".TBL_PAGES." where intPages_Id=".$_GET["id"];
					if($cnf->records_fetch($strSqlActive)==true) {
						...
						$strSqlUpdate = "update ".TBL_PAGES." set `cStatus`='".$strActive."' where intBookId=".$_GET["id"];
						$cnf->query_execute($strSqlUpdate);						
						
						header("Location:page.php?act=".$actmsg.$url_fields);
						} else {
						header("Location:page.php?act=Invalid".$url_fields);
					}
				}		
				if($_POST["btnSubmit"]!="") {				
				$selOptions = $_POST["selOptions"];				
					...
					for($intI=0;$intI<=$IntCount;$intI++) {
						...
						$strSqlDelete = "delete from ".TBL_PAGES." where intPages_Id=".$_POST["chk"][$intI];
						$cnf->query_execute($strSqlDelete);
						
						$strSql = "delete from ".TBL_PAGESCONT." where pages_id=".$_POST["chk"][$intI];
						$res = mysqli_query($con,$strSql);																			
						
						} else {
						$strSqlUpdateOptions = "update ".TBL_PAGES." set ".$strString." where intPages_Id=".$_POST["chk"][$intI]; }
				...
				if($_GET["act"]=="deactive" && $_REQUEST['rec'] != 'mul') {									
					...
					echo $cnf->HideMsg(); }
						...
						$filterstr = "%" .$filterstr ."%"; 
						   ...
						   $strSqlSelect = "SELECT `intPages_Id`, `pages_name`, `dtCreated`, `cStatus` FROM ".TBL_PAGES." "; 						  
						...
						$strSqlSelect .= "where (`pages_name` like '" .$filterstr ."') or (`dtCreated` like '" .$filterstr ."') ";	
						 }			 
						  
						  if($order!=''){$strSqlSelect .= " order by `" .$_SESSION["order"]."`";}else{$strSqlSelect .= " order by `intPages_Id` ";}
							...
							<div class="col-lg-12">
								...
								<section class="panel">
                                    ...
                                    <?php
										...
										$strQry = "SELECT * FROM `".TBL_PAGES."` ORDER BY `intPages_Id` DESC";
                                              ...
                                              <?php
												...
												?>
                                                    ...
                                                    <tr class="gradeX">
                                                        ...
                                                        <td><input type="checkbox" name="chk[]" value="<?php echo $rowQry->intPages_Id ?>" <?php  if($_GET["act"]=="CheckAll") { ?> checked <?php } ?> /></td>

                                                        <td><?php echo ($rowQry->pages_name); ?></td>
                                                        ...
                                                        <td class="center">
                                                            ...
                                                            <a href="edit_page.php?id=<?php echo $rowQry->intPages_Id.$url_fields ?>" onClick="return confirmMsg(1);" class="text-info" title="Edit"><i class="fa fa-pencil" ></i></a>
