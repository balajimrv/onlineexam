<?php include("includes/header.php");
	...
	############# Active #############
				if($_GET["act"]=="BanActive") {
					$strSqlActive = "select * from ".TBL_QUESTIONS." where question_id=".$_GET["id"];
					if($cnf->records_fetch($strSqlActive)==true) {
						...
						$strSqlUpdate = "update ".TBL_QUESTIONS." set `cStatus`='".$strActive."' where question_id=".$_GET["id"];
						$cnf->query_execute($strSqlUpdate);						
						
						header("Location:questions.php?act=".$actmsg.$url_fields);
						} else {
						header("Location:questions.php?act=Invalid".$url_fields);
					}
				}		
		...
		$strSql = "DELETE FROM ".TBL_QUESTIONS." where question_id=".$_GET["id"];

		$cnf->query_execute($strSql);

		header("Location:questions.php?act=deleted");
				...
				if($_GET["act"]=="active" && $_REQUEST['rec'] == 'mul') {									
					$strMsg = 'This questions are activated successfully';
					echo $cnf->HideMsg(); 
				}	
				if($_GET["act"]=="deactive" && $_REQUEST['rec'] == 'mul') {									
					$strMsg = 'This questions are deactivated successfully';
					echo $cnf->HideMsg(); 
				}	
				...
				if($_GET["act"]=="active" && $_REQUEST['rec'] != 'mul') {									
					$strMsg = "This questions activated successfully."; 
					echo $cnf->HideMsg(); }
					
				if($_GET["act"]=="deactive" && $_REQUEST['rec'] != 'mul') {									
					$strMsg = "This questions deactivated successfully."; 
					echo $cnf->HideMsg(); }
					...
					<header class="page-header">
						...
						<div class="right-wrapper pull-right">
							<ol class="breadcrumbs">
                                ...
                                <li><span><a href="addquestions.php" class="btn btn-success btn-small"><i class="fa fa-plus" aria-hidden="true"></i>
 Add New Question</a></span></li>
					...
					<div class="row">
							<div class="col-lg-12">
								...
								<section class="panel">
									...
									$strQry = "SELECT a.*, b.test_id, b.test_name from ".TBL_QUESTIONS." as a inner join ".TBL_TEST." as b on a.test_id=b.test_id";	  
									...
									$strQry .= " WHERE (`question_id` like '" .$filterstr ."') or (`question` like '" .$filterstr ."') or (`test_name` like '" .$filterstr ."')  or (`test_type` like '" .$filterstr ."') ";	
									}		
									
									
									if($order!=''){$strQry .= " order by `" .$_SESSION["order"]."`";}else{$strQry .= " order by `question_id` ";}
                                    ...
                                    <form action="questions.php" method="post">
                                        <table class="table table-bordered table-striped" width="100%" border="0" cellspacing="0" cellpadding="0" >
                                            <tr>
                                                ...
                                                &nbsp;<a href="questions.php?a=reset" class="btn btn-link">Reset List</a>
                                                </td>
                                            </tr>
                                        </table>
      								</form>
    
                                    <form action="questions.php" method="POST" id="actionfrm" class="form-horizontal form-bordered" name="frm">
                                    	<?php
										//$strQry = "SELECT * FROM `".TBL_QUESTIONS."` WHERE question_id=1 ORDER BY `question_id` DESC";
										...
										?>
                                            <table class="table table-bordered table-striped table-condensed" style="width:100%">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th width="508">Question</th>
                                                        <th width="112">Type</th>
                                                        <th width="300"><a href="questions.php?order=<?php echo "test_name" ?>&type=<?php echo $ordtypestr ?>&filter=<?php echo $_REQUEST['filter']?>&filterfield=<?php echo $_REQUEST['filterfield']?>&page=<?php echo $_REQUEST["page"];?>">Test Name</a></th>
                                                        
                                                        <th width="40"><a class="title_link" href="questions.php?order=<?php echo "cStatus" ?>&type=<?php echo $ordtypestr ?>&filter=<?php echo $_REQUEST['filter']?>&filterfield=<?php echo $_REQUEST['filterfield']?>&page=<?php echo $_REQUEST["page"];?>">Status</a></th>
												...
												?>
                                                    <tr class="gradeX">
                                                        ...
                                                        <td><input type="checkbox" name="chk[]" value="<?php echo $rowQry->question_id ?>" <?php  if($_GET["act"]=="CheckAll") { ?> checked <?php } ?> /></td>                        
                                                        
                                                        
                                                        <td><a href="#" data-toggle="modal" data-target="#myModal" id=<?php echo $rowQry->question_id; ?>><?php echo $cnf->custom_echo($rowQry->question, 70);?></a></td>
                                                        <td><?php echo $rowQry->answer_type;?></td>
                                                        <td><?php echo $rowQry->test_name;?></td>
                                                        <td align="center">
														<?php echo $cnf->getActiveImageHide($rowQry->cStatus,'questions.php?act=BanActive&id='.$rowQry->question_id); ?>
                                                        </td>
                                                      ...
                                                      if($_SESSION["userType"] != 2){ ?>
                                                          <a href="edit_questions.php?id=<?php echo $rowQry->question_id.$url_fields ?>" onClick="return confirmMsg(1);" class="text-info" title="Edit"><i class="fa fa-pencil" ></i></a>&nbsp;&nbsp;
                                                            
                                                            <a href="questions.php?act=delete&id=<?php echo $rowQry->question_id.$url_fields ?>" onClick="return confirmMsg(2);" class="text-danger" title="Delete"><i class="fa fa-trash-o"></i></a>
                                                     <?php
                                                      }else{ ?>
                                                    <a href="questions.php?act=delete&id=<?php echo $rowQry->question_id.$url_fields ?>" onClick="return confirmMsg(2);" class="text-danger" title="Delete"><i class="fa fa-trash-o"></i></a>  
                                                    <?php 
                                            ...
                                            <table class="table table-condensed table-striped">
                                                <tr>
                                                    ...
                                                    <td align="right" valign="middle"><?php  $cnf->showpagenav($page, $pagecount,"questions.php?order=".$_REQUEST["order"]."&type=".$_REQUEST["type"]."&filter=".$_SESSION['filter']."&filterfield=".$_SESSION['filterfield']."");?></td>
                                                </tr>
                                            </table>
                                            ...
                                            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="rgtlistbdr">
                                                <tr>
                                                    ...
                                                    <td width="200" align="left" valign="middle">
                                                        ...
                                                        <noscript>
                                                        <a href="questions.php?act=<?php echo $strCheck?>" class="text_btm_link"><?php echo $strCheckString?></a>
                                                        </noscript>
                                                    </td>
	...
	if($_POST["btnSubmit"]!="") {		
				...
				$selOptions = $_POST["selOptions"];					
					...
					for($intI=0;$intI<=$IntCount;$intI++) {
						...
						$strSqlDelete = "delete from ".TBL_QUESTIONS." where question_id=".$_POST["chk"][$intI];
						$cnf->query_execute($strSqlDelete);
						
						} else {
						$strSqlUpdateOptions = "update ".TBL_QUESTIONS." set ".$strString." where question_id=".$_POST["chk"][$intI]; }
					...
					header("Location:questions.php?act=".$strString."&rec=mul");
				}			
 ?>
