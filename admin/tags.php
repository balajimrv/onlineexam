<?php include("includes/header.php");
	...
	############# Active #############
				if($_GET["act"]=="btnActive") {
					$strSqlActive = "SELECT * FROM ".TBL_TAGS." WHERE tags_id=".$_GET["id"];
					if($cnf->records_fetch($strSqlActive)==true) {
						...
						$strSqlUpdate = "update ".TBL_TAGS." set `cStatus`='".$strActive."' where tags_id=".$_GET["id"];
						$cnf->query_execute($strSqlUpdate);						
						
						header("Location:tags.php?act=".$actmsg.$url_fields);
						} else {
						header("Location:tags.php?act=Invalid".$url_fields);
					}
				}	
		...
		if($_REQUEST['act']== 'deleterec'){
			$strSql = "DELETE FROM ".TBL_TAGS." WHERE tags_id=".$_REQUEST['id']." LIMIT 1";
			$cnf->query_execute($strSql);			
			header("Location:tags.php?act=deleted".$url_fields);
		}
				...
				$selOptions = $_POST["selOptions"];					
					...
					for($intI=0;$intI<=$IntCount;$intI++) {
						...
						$strSqlDelete = "delete from ".TBL_TAGS." where tags_id=".$_POST["chk"][$intI];
						$cnf->query_execute($strSqlDelete);						
						} else {
						$strSqlUpdateOptions = "update ".TBL_TAGS." set ".$strString." where tags_id=".$_POST["chk"][$intI]; }
					...
					header("Location:tags.php?act=".$strString."&rec=mul");
				}				
				...
				if($_GET["act"]=="deleted") {
					$strMsg = "Record has been deleted successfully";
					echo $cnf->HideMsg(); }
...
$strSqlSelect = "SELECT * FROM ".TBL_TAGS." "; 						  
...
$strSqlSelect .= "where (`tags` like '" .$filterstr ."') ";	
}			 

if($order!=''){$strSqlSelect .= " order by `" .$_SESSION["order"]."`";}else{$strSqlSelect .= " order by `tags_id` ";}
							...
							<div class="col-lg-12">
								...
								<section class="panel">
                                    ...
                                    <div class="pull-right navadminbtn" style="margin-bottom:10px;">
                                         <a href="addtags.php"><button class="btn btn-success">Add Tags</button></a>
                                    </div>
                                            ...
                                            <tr>
                                                <td align="center" valign="middle"><b>FILTER</b> :
                                                    ...
                                                    <select name="filterfield" class="input-sm">
                                                        ...
                                                        <option value="<?php echo "tags" ?>"<?php if ($filterfield == "tags") { echo "selected"; } ?>><?php echo htmlspecialchars("Tag Name") ?></option>
                                                    </select>
                                                    &nbsp;&nbsp;&nbsp;Whole words only
                                                ...
                                                &nbsp;<a href="tags.php?a=reset" class="btn btn-link">Reset List</a>
                                                </td>
                                            </tr>
                                        </table>
      								</form>
										...
										?>
                                            <table class="table table-bordered table-striped">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th><a href="tags.php?order=<?php echo "tags" ?>&type=<?php echo $ordtypestr ?>&filter=<?php echo $_REQUEST['filter']?>&filterfield=<?php echo $_REQUEST['filterfield']?>&page=<?php echo $_REQUEST["page"];?>">Tags Name</a></th>
                                                                                                               
                                                        <th><a class="title_link" href="tags.php?order=<?php echo "cStatus" ?>&type=<?php echo $ordtypestr ?>&filter=<?php echo $_REQUEST['filter']?>&filterfield=<?php echo $_REQUEST['filterfield']?>&page=<?php echo $_REQUEST["page"];?>">Status</a></th>
												...
												?>
                                                    <tr class="gradeX">
                                                        <td><?php echo $i+1; ?></td>
                                                        <td><input type="checkbox" name="chk[]" value="<?php echo $rowQry->tags_id ?>" <?php  if($_GET["act"]=="CheckAll") { ?> checked <?php } ?> /></td>                        
                                                        <td><?php echo $rowQry->tags; ?></td>
                                                        <td align="center">
														<?php echo $cnf->getActiveImageHide($rowQry->cStatus,'tags.php?act=btnActive&id='.$rowQry->tags_id)?>
                                                        </td>
                                                        
                                                        
                                                        <td align="center" valign="middle" class="brwhite">
													   <a href="edit_tags.php?id=<?php echo $rowQry->tags_id.$url_fields ?>" onClick="return confirmMsg(1);" class="text-info" title="Edit"><i class="fa fa-pencil" ></i></a>&nbsp;&nbsp;
                                                            
                                                            <a href="tags.php?act=deleterec&id=<?php echo $rowQry->tags_id.$url_fields ?>" onClick="return confirmMsg(2);" class="text-danger" title="Delete"><i class="fa fa-trash-o"></i></a>
                                                      </td>
                                                    </tr>
                                                <?php } ?>                                                     
                                                </tbody>
                                            ...
                                            <table class="table table-condensed table-striped">
                                                <tr>
                                                    ...
                                                    <td align="right" valign="middle"><?php  $cnf->showpagenav($page, $pagecount,"tags.php?order=".$_REQUEST["order"]."&type=".$_REQUEST["type"]."&filter=".$_SESSION['filter']."&filterfield=".$_SESSION['filterfield']."");?></td>
                                                </tr>
                                            </table>
                                            ...
                                            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="rgtlistbdr">
                                                <tr>
                                                    ...
                                                    <td width="200" align="left" valign="middle">
                                                        ...
                                                        <noscript>
                                                        <a href="tags.php?act=<?php echo $strCheck?>" class="text_btm_link"><?php echo $strCheckString?></a>
                                                        </noscript>
                                                    </td>
