<?php include("includes/header.php");
	...
	if($_GET["act"]=="btnActive") {
		$strSqlActive = "select * from ".TBL_USERS." where user_id=".$_GET["id"];
		...
		if($rs_contents->cStatus=='N') {
			...
			$strSqlUpdate = "update ".TBL_USERS." set `cStatus`='".$strActive."' where user_id=".$_GET["id"];
			$cnf->query_execute($strSqlUpdate);							
		
			header("Location:user.php?act=".$actmsg.$url_fields);
		}else{
			header("Location:user.php?act=Invalid".$url_fields);
		}
	}
	
	if($_GET["act"]=="emailActive") {
		$strSqlActive = "select * from ".TBL_USERS." where user_id=".$_GET["id"];
		if($cnf->records_fetch($strSqlActive)==true) {
			$rs_contents = mysqli_fetch_object($cnf->query_execute($strSqlActive));
		
		if($rs_contents->emailStatus=='N') {
			$strActive ='Y';
			$actmsg="active";				
		}else if($rs_contents->emailStatus=='Y') {
			$strActive ='N';
			$actmsg="deactive";				
		}							
		
			$strSqlUpdate = "update ".TBL_USERS." set `emailStatus`='".$strActive."' where user_id=".$_GET["id"];
			$cnf->query_execute($strSqlUpdate);							
	...
	if($_REQUEST['act']== 'delete'){
		$strSql = "DELETE FROM ".TBL_USERS." WHERE user_id=".$_REQUEST['id']." LIMIT 1";
		$cnf->query_execute($strSql);
		header("Location:user.php?act=deleted".$url_fields);
	}
	...
	if($_POST["btnSubmit"]!="") {				
		...
		for($intI=0;$intI<=$IntCount;$intI++) {
			if($selOptions=="deleteRec") {
				$strSqlDelete = "DELETE FROM ".TBL_USERS." WHERE user_id=".$_POST["chk"][$intI];
				$cnf->query_execute($strSqlDelete);																		
			}else{
				$strSqlUpdateOptions = "UPDATE ".TBL_USERS." SET ".$strString." WHERE user_id=".$_POST["chk"][$intI]; }
	...
	if($selOptions !="deleteRec") {
			...
			$strString = "deleted"; }
			header("Location:user.php?act=".$strString.$url_fields);
	}
...
?>
					...
					<header class="page-header">
						...
						<div class="right-wrapper pull-right">
							<ol class="breadcrumbs">
								...
								<!--<li><span><a href="adduser.php">Add User</a></span></li>-->
							</ol>
							...
							<div class="col-lg-12">
								...
								<section class="panel">
									...
									if ($wholeonly && $_SESSION["filterfield"]!='') {
										...
										$strQry .= "WHERE (`first_name` like '" .$filterstr ."') or (`intPhone` like '" .$filterstr ."') or (`email` like '" .$filterstr ."')";
										
									}			 
									
									if($order!=''){$strQry .= " ORDER BY `" .$_SESSION["order"]."`";}else{$strQry .= " ORDER BY `user_id` ";}
	...
	<option value="<?php echo "intPhone" ?>"<?php if ($filterfield == "intPhone") { echo "selected"; } ?>><?php echo htmlspecialchars("Phone") ?></option>
                                                ...
                                                &nbsp;<a href="user.php?a=reset" class="btn btn-link">Reset List</a>
                                                </td>
                                            </tr>
                                        </table>
      								</form>
                                    ...
                                    <form action="" method="POST" class="form-horizontal form-bordered" name="frm" onSubmit="return frmAction(frm);">
                                    	<?php
										//$strQry = "SELECT * FROM `".TBL_USERS."` WHERE user_id=1 ORDER BY `user_id` DESC";
										...
										?>
                                            <table class="table table-bordered table-striped">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th>Email Address</th>
                                                        <th>Phone</th>
                                                        <th><a href="user.php?order=<?php echo "dtCreated" ?>&type=<?php echo $ordtypestr ?>&filter=<?php echo $_REQUEST['filter']?>&filterfield=<?php echo $_REQUEST['filterfield']?>&page=<?php echo $_REQUEST["page"];?>">Join Date</a></th>
                                                        <th><a href="user.php?order=<?php echo "cStatus" ?>&type=<?php echo $ordtypestr ?>&filter=<?php echo $_REQUEST['filter']?>&filterfield=<?php echo $_REQUEST['filterfield']?>&page=<?php echo $_REQUEST["page"];?>">Status</a></th>
                                                        <th><a href="user.php?order=<?php echo "cStatus" ?>&type=<?php echo $ordtypestr ?>&filter=<?php echo $_REQUEST['filter']?>&filterfield=<?php echo $_REQUEST['filterfield']?>&page=<?php echo $_REQUEST["page"];?>">Email Status</a></th>
                                                        <th>Action</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
												...
												?>
                                                    <tr class="gradeX">
                                                        <td><?php echo $i+1; ?></td>
                                                        <td><input type="checkbox" name="chk[]" value="<?php echo $rowQry->user_id ?>" <?php  if($_GET["act"]=="CheckAll") { ?> checked <?php } ?> /></td>
                                                      
                                                        <td><a href="#" data-toggle="modal" data-target="#myModal" id=<?php echo $rowQry->user_id; ?>><?php echo $rowQry->first_name; ?></a></td>
                                                        <td><?php echo $rowQry->email; ?></td>
                                                        <td><?php echo $rowQry->phone; ?></td>
                                                        <td class="center"><?php echo $cnf->convetdate($rowQry->dtCreated); ?></td>
                                                        <td class="center"><?php echo $cnf->getActiveImageHide($rowQry->cStatus,'user.php?act=btnActive&id='.$rowQry->user_id)?></td>
                                                        <td class="center"><?php echo $cnf->getActiveImageHide($rowQry->emailStatus,'user.php?act=emailActive&id='.$rowQry->user_id)?></td>
                                                        <td class="center">
                                                        	
                                                            <a href="edit_user.php?id=<?php echo $rowQry->user_id.$url_fields ?>" onClick="return confirmMsg(1);" class="text-info" title="Edit"><i class="fa fa-pencil" ></i></a>&nbsp;&nbsp;
                                                            <?php if($_SESSION["intAdminId"] == 1){ ?>
                                                            <a href="user.php?act=delete&id=<?php echo $rowQry->user_id.$url_fields ?>" onClick="return confirmMsg(2);" class="text-danger" title="Delete"><i class="fa fa-trash-o"></i></a>
                                                            <?php } ?>
                                                        </td>
                                                    </tr>
                                                  <?php } ?>                                                     
                                            ...
                                            <table class="table table-condensed table-striped">
                                                <tr>
                                                    ...
                                                    <td align="right" valign="middle"><?php  $cnf->showpagenav($page, $pagecount,"user.php?order=".$_REQUEST["order"]."&type=".$_REQUEST["type"]."&filter=".$_SESSION['filter']."&filterfield=".$_SESSION['filterfield']."");?></td>
                                                </tr>
                                            </table>
                                            ...
                                            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="rgtlistbdr">
                                                <tr>
                                                    ...
                                                    <td width="200" align="left" valign="middle">
                                                        ...
                                                        <noscript>
                                                        <a href="user.php?act=<?php echo $strCheck?>" class="text_btm_link"><?php echo $strCheckString?></a>
                                                        </noscript>
                                                    </td>
